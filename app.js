...
const apiKey = "c7e3decf49ff44cebe99eb6c674a1f72";
...
form.addEventListener("submit", async function (event) {
  ...
  if (data.status !== "ok") {
    alertaContainer.innerHTML = `<div class="alert alert-danger">Error: ${data.message}</div>`;
    return;
  }
  if (!data.articles || data.articles.length === 0) {
    alertaContainer.innerHTML = `<div class="alert alert-info">No se encontraron noticias.</div>`;
    return;
  }
      ...
      col.innerHTML = `
                <div class="card h-100">
                    ...
                    <div class="card-footer">
                        ...
                        <br>
                       <a href="${noticia.url}" class="btn btn-sm btn-secondary mt-2" target="_blank">Ver</a>
                    </div>
                </div>
            `;
      row.appendChild(col);
